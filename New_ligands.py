...
try: from urslib import Tools
except ImportError: import Tools

def run_all(infolder=''):

    good_input = 0

    while not good_input:
        
        if os.path.exists(infolder): good_input = 1

        else: infolder  = input('Please enter the path to folder with new pdb files: ')

    if infolder[-1] != '/': infolder = infolder + '/'
...
class Model():
    ...
    def parse(self, modelname):
        ...
        for string in PDBdict['TITLE']: Title += string[10:80]
        ...
        for ch in self.chains:
            ...
            for residue in self.chains[ch]['SEQ'][:check]:
                ...
                if residue not in Tools.restype: print(residue)
        ...
        ter  = {}
        seen = {}
        for ch in self.chains: ter[ch] = False
            ...
            if line[:6] in ('ATOM  ','HETATM'):
                ...
                if chain not in self.chains: # if atom don't belong to any chain
                    ...
                    if chain + atom['RESNUM'] not in seen:
                        ...
                        if res['NAME'] not in Tools.restype: print(res['NAME'])
        ...
        for ch in self.chains:
            ...
            while misseq:
                ...
                if misres['NAME'] not in Tools.restype: print(misres['NAME'])
